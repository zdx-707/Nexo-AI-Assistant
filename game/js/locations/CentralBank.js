window.CentralBank = class CentralBank extends window.Location {
  constructor(scene) {
    ...
    this.depth = 55;
    this.wallHeight = 9;
    this.wallColor = 0xd4c9a8;
    this.floorColor = 0xe8e0d0;
  }

  buildInterior() {
    // Vault server room: stacked crates & control panels
    this.addStorageCrates(-22, -20, 3, 4);
    this.addStorageCrates(18, -20, 3, 3);
    this.addControlPanel(-20, -5, Math.PI / 2);
    this.addControlPanel(18, -5, -Math.PI / 2);
    this.addControlPanel(0, -22, 0);

    // Security cage (partition walls)
    this.addPartition(-8, -8, 14, true);
    this.addPartition(-8, -22, 14, true);
    this.addPartition(-8, -8, 14, false);
    this.addPartition(6, -8, 14, false);

    // Cash processing barrels (coolant)
    this.addBarrels(-20, 10, 4, 0.7);
    this.addBarrels(14, 10, 4, 0.7);

    // Teller wing (small office area)
    this.addPartition(-5, 5, 16, true);
    this.addPartition(-13, 5, 8, false);
    [[-10,8,0],[-10,2,0]].forEach(([x,z,y]) => this.addDesk(x,z,y));
    [[-9,8],[-9,2]].forEach(([x,z]) => this.addEmployee(x,z,Math.PI));
    this.addFilingCabinets(-22, 5, 3, 0);

    // Manager's office
    this.addPartition(8, 10, 10, true);
    this.addPartition(8, 10, 7, false);
    this.addDesk(14, 14, Math.PI);
    this.addEmployee(13, 14, Math.PI);

    // Security desk at entrance
    const sd = this.loader.createDesk(0x5a4010);
    sd.position.set(0, 0, 22); this.scene.add(sd); this.meshes.push(sd);
    const sm = this.loader.createMonitor();
    sm.position.set(0, 0.78, 21.7); sm.rotation.y = Math.PI;
